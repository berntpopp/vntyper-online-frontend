...
export function initializeServerLoad() {
  ...
  function adjustServerLoadUpdateInterval(totalJobsInQueue) {
    ...
    if (totalJobsInQueue > 0) {
      ...
      logMessage(
        `Adjusting polling interval to ${intervalTime / 1000} seconds due to active jobs in the queue.`,
        'info'
      );
    } else {
      logMessage(
        `Adjusting polling interval to ${intervalTime / 1000} seconds as the queue is empty.`,
        'info'
      );
    }
