...
export class ErrorHandler {
    ...
    handleError(error, context = {}, level = ErrorLevel.ERROR) {
        ...
        const consoleMethod = level === ErrorLevel.CRITICAL ? 'error' : level === ErrorLevel.WARNING ? 'warn' : 'log';
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    logMessage(`Retry ${attempt + 1}/${maxRetries} after ${delay}ms: ${error.message}`, 'warning');
