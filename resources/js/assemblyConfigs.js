...
export function getNcbiAccession(coordinateSystem, chromosomeNumber) {
  ...
  if (!assembly) {
    throw new Error(
      `Unknown coordinate system: ${coordinateSystem}. Must be "GRCh37" or "GRCh38".`
    );
  }
  ...
  if (!contig || !contig.ncbiAccession) {
    throw new Error(
      `No NCBI accession for chromosome ${chromosomeNumber} in ${coordinateSystem}. Valid range: 1-24 (23=X, 24=Y).`
    );
  }
