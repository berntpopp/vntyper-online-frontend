...
describe('CohortController', () => {
  ...
  describe('handleCreate() - Successful creation', () => {
    ...
    it('should generate default alias if not provided', async () => {
      ...
      mockAPIService.createCohort.mockResolvedValue(mockCohort)
      vi.spyOn(cohortController, 'generateDefaultAlias').mockReturnValue('Cohort-2025-10-09T12-00-00-000Z')
      ...
      expect(mockAPIService.createCohort).toHaveBeenCalledWith('Cohort-2025-10-09T12-00-00-000Z', undefined)
    })
    ...
    it('should update state with cohort data', async () => {
      ...
      // Assert
      expect(mockStateManager.addCohort).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', {
        alias: 'Test Cohort',
        hasPassphrase: false,
        jobIds: [],
    ...
    it('should show cohort in view with shareable link', async () => {
      ...
      // Assert
      expect(mockCohortView.showCohort).toHaveBeenCalledWith(
        mockCohort,
  ...
  describe('handleCreate() - Error handling', () => {
    it('should handle cohort creation errors', async () => {
      ...
      // Act & Assert
      await expect(
        cohortController.handleCreate({ alias: 'Test Cohort' })
      ).rejects.toThrow('Cohort creation failed')
    ...
    it('should log cohort creation errors', async () => {
      ...
      // Act & Assert
      await expect(
        cohortController.handleCreate({ alias: 'Test' })
      ).rejects.toThrow()
  ...
  describe('handleAddJob()', () => {
    it('should add job to cohort in state', () => {
      ...
      // Assert
      expect(mockStateManager.addJobToCohort).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', 'job-123')
    })
    ...
    it('should update cohort view with job count', () => {
      ...
      // Assert
      expect(mockCohortView.updateCohort).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', {
        jobCount: 3
      })
    })
  ...
  describe('handleJobCompleted()', () => {
    ...
    it('should show analysis section when all jobs complete', () => {
      ...
      // Assert
      expect(mockCohortView.showAnalysisSection).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d')
    })
  ...
  describe('handlePoll()', () => {
    ...
    it('should call getCohortStatus via poll function', async () => {
      ...
      // Assert
      expect(mockAPIService.getCohortStatus).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', 'secret')
    })
    ...
    it('should store stop function in state', async () => {
      ...
      // Assert
      expect(mockStateManager.setCohortPolling).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', mockStopFn)
    })
    ...
    it('should handle polling setup errors', async () => {
      ...
      // Assert
      expect(mockLogger.logMessage).toHaveBeenCalledWith(
        expect.stringContaining('[CohortController] Polling failed for cohort c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d'),
        'error'
      )
    })
  ...
  describe('handleAnalyze()', () => {
    it('should update analysis status in view', async () => {
      ...
      // Assert
      expect(mockCohortView.updateAnalysisStatus).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d', 'processing')
    })
  ...
  describe('getCohort()', () => {
    it('should return Cohort model when cohort exists', () => {
      ...
      // Assert
      expect(mockStateManager.getCohort).toHaveBeenCalledWith('c9b7f8d3-4e5a-4f6b-8c7d-9e0f1a2b3c4d')
  ...
  describe('Integration Workflow', () => {
    it('should handle complete cohort creation and job assignment workflow', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('cohort:created', expect.objectContaining({
    ...
    it('should handle cohort polling and completion workflow', async () => {
      ...
      // Assert - Final state
      expect(mockEventBus.emit).toHaveBeenCalledWith('cohort:completed', expect.objectContaining({
        cohortId: expect.any(String)
      }))
    })
  })
